...
def get_grants(schema_name, table_name, current_user):
    sql = '''
        ...
        AND aclcontains(c.relacl, makeaclitem(usr.usesysid::int, pg_user.usesysid::int, priviledge."grant", false))
        AND c.relname = '%s'
        AND nc.nspname = '%s'
        and usr.usename != '%s'
        ;
...
def reduce_column_length(col_type, column_name, table_name):
    ...
    if "varchar" in col_type:
        curr_col_length = STRING_REDUCTION_MAX_LENGTH_THRESHOLD #int(re.search(r'\d+', col_type).group())
    ...
    if "varchar" in col_type:
        ...
        if new_column_len < curr_col_length:
            set_col_type = col_type + '(' + str(new_column_len) + ')' #re.sub(str(curr_col_length), str(new_column_len), col_type)
            comment(str(curr_col_length))
            comment(str(new_column_len))
            comment(str(col_type))
            comment(str(set_col_type))
    else:
...
def analyze(table_info):
    ...
    owner = 'datapipeline_app' #table_info[5]
    if len(table_info) > 6:
...
''' % (schema_name, table_name)
        ...
        try:
            ...
            if debug:
                comment(str(analyze_compression_result))
            ...
            for row in analyze_compression_result:
                ...
                if str(col_null).upper() == 'TRUE' and 1 == 0:
                    col_null = 'NOT NULL'
                else:
                    col_null = ''
                ...
                if default_or_identity and 1 == 0:
...
def run():
    ...
    if table_name is not None:
        statement = '''select pgn.nspname::text as schema, trim(pgc.relname) as table, pg_total_relation_size(pgc.oid)/1024/1024 as mbytes, pgc.reltuples as rows
    , (case when pgc.reltuples <= 100000 then 'ALL' else 'KEY' end) as dist_style, TRIM(pgu.usename) "owner", pgd.description
...
where pgn.nspname::text ~ '^%s$' and pgc.relname in (%s);
        ''' % (schema_name, tables)
    else:
...
join pg_namespace as pgn on pgn.oid = pgc.relnamespace
join pg_user pgu on pgu.usesysid = pgc.relowner
where pgn.nspname::text ~ '^%s$' and pgc.relname::text SIMILAR TO '[A-Za-z0-9_]*'
order by 2;
        ''' % (schema_name,)
